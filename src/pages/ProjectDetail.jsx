...
function ProjectDetail() {
  ...
  const financialCards = [
    ...
    { label: "Revised Contract Value", value: project.revisedContractValue,  color: "text-slate-900" },
    ...
    { label: "Remaining to Invoice",   value: project.remainingToInvoice,    color: "text-slate-600"  },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          {isAdmin && (
            <button
              onClick={openEdit}
              className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-700 hover:bg-slate-50 transition shadow-sm text-sm font-medium"
            >
              <Edit2 size={16} />
              Edit
            </button>
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">{project.clientName ?? project.client?.name ?? ""}</p>
              </div>
            </div>
        ...
        <div className="px-6 py-5 border-b border-slate-100">
          <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-sm text-slate-500">{Math.round(paidPct)}% paid</p>
          </div>
          <div className="flex h-3 rounded-full overflow-hidden bg-slate-100">
            ...
            <div className="bg-slate-200 h-full transition-all" style={{ width: `${remainingPct}%` }} title={`Remaining to invoice: ${Math.round(remainingPct)}%`} />
          </div>
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        {changeOrders.length === 0 ? (
          <div className="text-center py-10 text-slate-400 text-sm">No change orders yet.</div>
        ) : (
          <div className="divide-y divide-slate-100">
            {changeOrders.map((co, i) => (
              <div key={co.id ?? i} className="px-6 py-4 flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-medium text-slate-900">{co.description}</p>
                  {co.createdAt && <p className="text-xs text-slate-400 mt-0.5">{fmtDate(co.createdAt)}</p>}
                </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {project.invoices.map(inv => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3 text-slate-600">{fmtDate(inv.issueDate)}</td>
                    <td className="px-6 py-3">
                      <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-xs font-medium">
                        {inv.status}
                      </span>
                    </td>
                    <td className="px-6 py-3 text-right font-medium text-slate-900">{fmt.format(inv.total ?? inv.totalAmount ?? 0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-200 w-full max-w-lg p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowEdit(false)} className="p-2 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-100 transition">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowEdit(false)} className="px-4 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition">
                Cancel
              </button>
              <button
      ...
      {showCO && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-200 w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold text-slate-900">Add Change Order</h3>
              <button onClick={() => setShowCO(false)} className="p-2 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-100 transition">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleAddChangeOrder} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mb-1">Use negative value for deductions.</p>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">₦</span>
                  <input
                    required
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setShowCO(false)} className="px-4 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition">
                  Cancel
                </button>
                <button
                  type="submit"
